...
.delete('/api/users/user/:id',(req,res)=>{
    ...
    try{
        if (usrid==1) {
            deleteUser.deleteUser(usrid); //tester
            res.status(200).json({
                status:200,
                message: "user enleve",
                user: usrid
    ...
    const curs = await client.db("nymous").collection("messages").deleteOne(id_user_dans_bdd:usrid);
    //const res1 = curs.toArray();
    //console.log(res1);
    */
    ...
    try{
        ...
        /*vérifier que l'utilisateur existe */
            ...
            res.json({id: id_user,
                ...
                status: 201});
            NewMessage = {
                author: req.params.id, //pour l'instant id mais il faut requete bdd pour avoir login
                message: message,
...
.put('/api/messages/users/user/:id/messages',(req,res)=>{
    ...
    /*
    async function getMessageID(id_user,text){
        query = {id:{$eq:id_user},
                text:{$eq:text}} // id est le nom dans la bdd
        //projection ?
        const cursor2=await client.db("nymous").collection("messages").find(query) //projection si besoin
        const res=cursor2.toArray();
        console.log(res);
        return res;
    }

    const id = getMessageID(id_user,message);
    const curs = await client.db("nymous").collection("messages").updateOne(id_user_dans_bdd:id);
    const res1 = curs.toArray();
    console.log(res1);
    */
...
.delete('/api/messages/user/user:id/messages',(req,res)=>{
    ...
    async function getMessageID(id_user,text){
        query = {id:{$eq:id_user},
                text:{$eq:text}} // id et text sont les noms dans la bdd
        //projection ?
        const cursor2=await client.db("nymous").collection("messages").find(query) //projection si besoin
        const res=cursor2.toArray();
        console.log(res);
    ...
    // fonction a mettre dans messages.js car on s'en sert plusieurs fois
    // const id = await messages.getMessageID(id_user,message); car fct dans messages.js
    const id = getMessageID(id_user,message);
    const curs = await client.db("nymous").collection("messages").delete(id_user_dans_bdd:id);
    const res1 = curs.toArray();
    console.log(res1);
    */
